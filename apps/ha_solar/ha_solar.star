...
Summary: Pulls Production and Consumption data from Home Assistant
Description: Single screen with animations showing Panel Production, Home Consumption and energy sent to grid
Author: tavdog, marcbaier, motoridersd
"""
...
DUMMY_DATA = {
    ...
    "currentPowerConsumption": 847.0,
    "currentPvGeneration": 397.0,
    "soc": 0,
    "consumption": 1000.96,
    "production": 3000.11,
}
...
def main(config):
    ...
    PVGeneration_entity_id = config.get("PVGeneration_entity_id")
    consumption_entity_id = config.get("consumption_entity_id")
    token = config.get("auth")
    PvGeneration_entity_status = get_entity_status(ha_server, PVGeneration_entity_id, token)
    consumption_entity_status = get_entity_status(ha_server, consumption_entity_id, token)
    currentPvGeneration = PvGeneration_entity_status["state"]
    currentPowerConsumption = consumption_entity_status["state"]

    has_battery = False  #  assume no battery until we have data

    data = dict()
    data["currentPowerConsumption"] = float(consumption_entity_status["state"])
    data["currentPvGeneration"] = float(PvGeneration_entity_status["state"])
...
def get_schema():
    return schema.Schema(
        ...
        fields = [
            ...
            schema.Text(
                id = "PVGeneration_entity_id",
                name = "Panel Generation Entity ID",
                icon = "user",
                desc = "Entity ID of the Solar Generation in W from Home Assistant",
            ),
            schema.Text(
                id = "consumption_entity_id",
                name = "Energy Consumption Entity ID",
                icon = "user",
                desc = "Entity ID of the Energy Consumption of the home in W from Home Assistant",
            ),
            schema.Text(
                id = "auth",
                name = "Bearer Token",
